...
| **Full suite command** | `npm test && npm run test:e2e -- e2e/flashquery-happy-path.spec.ts e2e/flashquery-vault-browse.spec.ts e2e/flashquery-disconnect.spec.ts e2e/flashquery-persistence.spec.ts e2e/fixtures/flashquery-server.spec.ts` |
| **Estimated runtime** | ~20 seconds for full unit suite, ~7 seconds for focused E2E, ~12 seconds for full FlashQuery E2E |
...
- **Before `$gsd-verify-work`:** Full suite plus focused persistence E2E and full FlashQuery E2E must be green.
...
| 10.1.2 | 10.1 | 1 | REQ-010, REQ-024 | T-U-007 | E2E-only preload helpers are absent in production and present only under `CATE_E2E=1`; renderer harness remains E2E-gated. | unit/static | `npm test -- src/preload/index.test.ts src/renderer/lib/e2eHarnessGate.test.ts && rg -n "CATE_E2E|__cateE2E|e2eHarness" src/preload/index.ts src/renderer/App.tsx src/renderer/lib/e2eHarness.ts src/renderer/lib/e2eHarnessGate.ts e2e/fixtures/electron-app.ts src/shared/electron-api.d.ts` | yes | green |
...
| 10.2.1 | 10.2 | 2 | REQ-005, REQ-009, REQ-024 | T-U-004, T-U-008, T-U-009, T-E-004 | Raw bearer tokens do not serialize to renderer/project/session persistence; pre-merge fixtures restore sanitized metadata; persistence E2E reads `.cate` files and asserts no token on disk. | unit/e2e | `npm test -- src/shared/types.test.ts src/main/flashquery/credentials.test.ts src/renderer/lib/session.test.ts && npm run test:e2e -- e2e/flashquery-persistence.spec.ts` | yes | green |
...
| 10.3.1 | 10.3 | 3 | REQ-024, REQ-025 | T-A-002, T-A-004, T-E-004 | Final build, typecheck, unit, focused persistence E2E, and full FlashQuery E2E evidence exists and exits 0. | build/unit/e2e | `npm run build && npm run typecheck && npm test && npm run test:e2e -- e2e/flashquery-happy-path.spec.ts e2e/flashquery-vault-browse.spec.ts e2e/flashquery-disconnect.spec.ts e2e/flashquery-persistence.spec.ts e2e/fixtures/flashquery-server.spec.ts` | yes | green |
