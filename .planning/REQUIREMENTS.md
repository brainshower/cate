...
- [ ] **REQ-001**: User can rely on graph-aware semantic connection types that cover all FlashQuery graph relation types, edge metadata, node metadata, community context, source chunk ordering, typed/mixed/embeddings-only modes, and optional scores.
...
- [ ] **REQ-003**: User can receive normalized FlashQuery `get_document` graph connection payloads, source chunks, graph summaries, and target health-tier fields through validated Cate IPC without losing useful partial data.
- [ ] **REQ-004**: User can see safe, complete display metadata for every graph edge relation, including directed/symmetric labels and unknown-relation fallbacks.
...
- [ ] **REQ-007**: User can see whole-document community context when graph community data is available, with empty community chrome hidden.
- [ ] **REQ-008**: User can see a whole-document attention area for active contradictions, open questions, and uncertainty without nature filters hiding critical findings.
- [ ] **REQ-009**: User can use a graph-enhanced sections list ordered by source chunks, with contradiction/question/certainty indicators and section selection.
- [ ] **REQ-010**: User can inspect typed whole-document connections grouped and sorted by relation priority, while embeddings-only results remain flat and mixed-mode untyped rows are retained.
- [ ] **REQ-011**: User can apply Top-N allocation, sort mode, and relation/nature filters to connection lists without affecting attention or section rows.
...
- [ ] **REQ-016**: User can open a toolbar-local text filter that persists across whole-document/selection views, clears with Escape/close, and never calls FlashQuery.
- [ ] **REQ-017**: User can filter by the specified data-bearing connection and section fields using case-insensitive substring matching, without structural UI text affecting matches.
...
- [ ] **REQ-021**: User can recover from loading, unsupported context, FlashQuery, adapter, malformed-result, and partial graph metadata states without renderer crashes or premature clean-state claims.
...
|-------------|-------|--------|---------------|
| REQ-001 | Phase 27 | Pending | T-U-001, T-U-002, T-U-003, T-U-026, T-A-002 |
| REQ-002 | Phase 27 | Pending | T-U-004, T-U-005, T-C-001, T-C-002, T-C-021, T-E-004 |
| REQ-003 | Phase 27 | Pending | T-I-001, T-I-002, T-I-003, T-I-004, T-I-007, T-I-008, T-A-002 |
| REQ-004 | Phase 27 | Pending | T-U-006, T-U-007, T-U-008 |
...
| REQ-010 | Phase 27 | Pending | T-U-017, T-U-018, T-C-018, T-C-019, T-C-020, T-C-021, T-C-064, T-E-001 |
| REQ-011 | Phase 27 | Pending | T-C-022, T-C-023, T-C-024, T-C-025 |
...
| REQ-021 | Phase 28 | Pending | T-U-016, T-U-026, T-U-028, T-C-003, T-C-004, T-C-063, T-I-003, T-E-005 |
