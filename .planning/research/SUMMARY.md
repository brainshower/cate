...
## Implications for Future Milestone Design

No roadmap phase is defined yet. The notes below are sequencing guidance to use only when the project owner explicitly creates a milestone or asks for phase planning.

### Connection Boundary And Health
...
### Explicit Save To Memory And Documents
...
### Native Workflow Polish And Hardening
...
Areas likely needing deeper research during future milestone planning:
- **Connection boundary:** Research exact MCP SDK client import paths, FlashQuery `/token` behavior, auth-expiry responses, and Cate's safest existing credential storage conventions.
- **Search/open:** Research Cate's current filesystem grant/realpath validation helpers and panel-opening APIs before implementing vault document opens.
- **Agent context:** Research current Pi agent prompt/session flow and `agentKey` routing before injecting context.

Areas with standard patterns once adopted by a milestone:
- **Save flows:** Save preview/confirmation, Zod validation, and MCP `callTool` wrappers are standard once connection and search/open contracts exist.
- **Polish/hardening:** Command palette entries, panel registration, renderer stores, and Playwright smoke tests follow existing Cate patterns.
...
- MCP SDK/package drift: verify current import paths and transport APIs before implementing the connection boundary.
...
- Agent prompt insertion point: confirm the least invasive Pi integration point before implementing agent context attachment.
