...
| REQ-032 | Passed | `VaultBadge` reuses `ChipSurface` and is wired into docked/canvas and detached editor chrome; local editors render no badge. Covered by T-I-094, T-I-095, T-I-096, T-I-098. |
...
- `npx -p node@22 npm test -- src/renderer/panels/EditorPanel.test.tsx src/renderer/components/Chip.test.tsx src/renderer/components/VaultBadge.test.tsx src/renderer/docking/DockTabBar.test.tsx src/renderer/shells/PanelWindowShell.test.tsx src/main/flashquery/clientManager.test.ts` — Passed, 68 tests.
- `npx -p node@22 npm run typecheck` — Passed.
- `npx -p node@22 npm test` — Passed, 51 files, 492 passed, 3 skipped.
...
- Dock and detached shell badge wiring is covered by source-level tests because full component imports hung under jsdom. Badge rendering and tooltip behavior are directly covered by `VaultBadge.test.tsx`.
