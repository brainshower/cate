...
- [24-04] Required grep `! rg -n "CustomEvent|preview-section-select" src/renderer src/shared` matches pre-existing non-preview CustomEvent paths in editor save/title and FlashQuery vault code. Preview-selection implementation did not introduce `CustomEvent` or `preview-section-select`.
