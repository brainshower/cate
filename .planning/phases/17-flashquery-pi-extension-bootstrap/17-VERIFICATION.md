...
| 4 | Tool registration includes plain host-filtered `tools/list` records and enriched `hostEligible: true` records with current statuses. | VERIFIED | `registry.ts` trusts no-metadata `tools/list` records, and requires `final`, `transitional`, or legacy `current` status plus top-level or metadata `hostEligible === true` for enriched records; unit tests cover plain records, final/transitional/current, deprecated, unavailable, removed, hidden, unknown-status, status-only, hostEligible-only, and missing-hostEligible records. |
